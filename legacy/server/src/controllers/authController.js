...
export const registerUser = async (req, res, next) => {
  try {
    ...
    const requireVerify = String(process.env.EMAIL_VERIFICATION_REQUIRED).toLowerCase() !== "false"
    if (requireVerify) {
    ...
    res.status(201).json({
      ...
      data: {
        ...
        requiresEmailVerification: requireVerify,
      },
    })
  } catch (error) {
    next(error)
...
export const registerDriver = async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      message: requireVerify
        ? "Driver registration successful. Your application is under review."
