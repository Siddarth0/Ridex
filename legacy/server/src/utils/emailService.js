...
export const sendEmail = async ({ to, subject, template, data, html, text }) => {
  try {
    if (String(process.env.EMAIL_SENDING_DISABLED).toLowerCase() === "true") {
      console.log(`[email] sending disabled: to=${to}, subject=${subject}`)
...
export const sendBulkEmails = async (emails) => {
  const results = []
  if (String(process.env.EMAIL_SENDING_DISABLED).toLowerCase() === "true") {
    console.log(`[email] bulk sending disabled: count=${emails?.length || 0}`)
    return emails.map(() => ({ messageId: "disabled" }))
  }
  const transporter = await createTransporter()
...
export const verifyEmailConfig = async () => {
  try {
    if (String(process.env.EMAIL_SENDING_DISABLED).toLowerCase() === "true") {
      console.log("[email] config verification skipped (sending disabled)")
      return true
    }
    const transporter = await createTransporter()
